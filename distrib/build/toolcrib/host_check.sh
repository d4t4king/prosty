...
#
# The ...M(ax) variables are used only for packages that can't (shouldn't) be
...
echo
echo "Checking host for required packages and functionality."
echo
...
if [ $? -eq 0 ]; then
  ...
  echo "  FAIL: can't compile."
  OK=OK+1
fi
rm -f dummy.c dummy

# Exit if all is well.
if [ $OK -eq 0 ]; then echo; echo; exit 0; fi

# Report some detail if !OK.
echo
case $OK in
  1) echo "One failure was detected." ;;
  *) echo "$OK failures were detected." ;;
esac

echo -e "\nYou must fix the failure(s) before you can build Smoothwall Express.\n"
echo "The toolcrib/host_*_inst script for your GNU/Linux distribution will"
echo "generally fix these problems. If there isn't one for your distro, modify one"
echo "or fix the problem(s) manually."
echo
echo "build_progress FAILED"
exit 255
